...
type JobDetailShareProps = {
  label: string
  className?: string
  /** Optional override for the URL to share. Defaults to the current page URL. */
  url?: string
  /** Optional title text for social share messages. */
  title?: string
}

export function JobDetailShare({ label, className, url, title }: JobDetailShareProps) {
  const getShareUrl = () => {
    if (url) return url
    ...
    <div className={cn("flex flex-wrap items-center gap-4", className)}>
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className="flex size-10 items-center justify-center rounded-full bg-[#40A0CA] text-white transition-all duration-200 hover:scale-105 hover:bg-[#006EA8] cursor-pointer"
        >
